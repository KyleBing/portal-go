...
export const API_MODULES: ApiModule[] = [
    {
        ...
        endpoints: [
            { method: 'POST', path: '/user/register', auth: 'P', summary: '注册（已有用户时需邀请码；首个用户为管理员且自动验证邮箱；其余用户需邮件验证）', body: 'email, password, username, nickname, invitationCode, …' },
            { method: 'POST', path: '/user/login', auth: 'P', summary: '登录；未验证邮箱将被拒绝；返回用户信息与 token（JWT）', body: 'email, password' },
            { method: 'GET', path: '/user/verify', auth: 'P', summary: '邮件链接验证邮箱（HTML）', params: 'token' },
            { method: 'POST', path: '/user/resend-verify', auth: 'P', summary: '重发验证邮件', body: 'email' },
            { method: 'POST', path: '/user/forgot', auth: 'P', summary: '申请重置密码邮件（防枚举，统一成功文案）', body: 'email' },
            { method: 'GET', path: '/user/reset', auth: 'P', summary: '重置密码页（HTML 表单）', params: 'token' },
            { method: 'POST', path: '/user/reset', auth: 'P', summary: '提交新密码', body: 'token, password' },
            { method: 'POST', path: '/user/force-verify', auth: 'Admin', summary: '强制标记邮箱已验证', body: 'uid' },
            { method: 'POST', path: '/user/send-reset-password', auth: 'Admin', summary: '向指定用户发送重置密码邮件', body: 'uid' },
            ...
            { method: 'POST', path: '/user/add', auth: 'P', summary: '创建用户（当前无鉴权；管理员添加视为已验证）', body: '同注册 + group_id', notes: '注意：接口公开' },
