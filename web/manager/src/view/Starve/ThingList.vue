<template>
    <Container>
        <Toolbar>
            ...
            <template #right>
                <ElButton v-if="isAdmin" type="success" icon="Plus" @click="addNewThing()"> 添加</ElButton>
            </template>
        </Toolbar>
        ...
        <ElDialog
            ...
            <ElForm
                ...
                <ElRow :gutter="20">
                    <ElCol :span="12">
                        <ElFormItem label="名称（中文）" prop="name">
                            <ElInput v-model="formThing.name" placeholder="请输入中文名称"/>
                        </ElFormItem>
                    </ElCol>
                    <ElCol :span="12">
                        <ElFormItem label="名称（英文）" prop="name_en">
                            <ElInput v-model="formThing.name_en" placeholder="请输入英文名称"/>
                        </ElFormItem>
                    </ElCol>
                </ElRow>
                <ElFormItem label="版本" prop="version">
                    <ElSelect 
                        v-model="formThing.version" 
                        multiple 
                ...
                <ElFormItem label="备注" prop="note">
                    <ElInput type="textarea" :rows="4" v-model="formThing.note" placeholder="请输入备注"/>
                </ElFormItem>
                <ElRow :gutter="20">
                    <ElCol :span="12">
                        <ElFormItem label="调试代码" prop="debugspawn">
                            <ElInput v-model="formThing.debugspawn" placeholder="请输入调试代码"/>
                        </ElFormItem>
                    </ElCol>
                    <ElCol :span="12">
                        <ElFormItem label="图片路径" prop="pic">
                            <ElInput v-model="formThing.pic" placeholder="请输入图片路径"/>
                        </ElFormItem>
                    </ElCol>
                </ElRow>
                <ElFormItem label="缩略图路径" prop="thumb">
                    <ElInput v-model="formThing.thumb" placeholder="请输入缩略图路径"/>
                </ElFormItem>
            </ElForm>
...
import { Thing, Version } from "@/model/starve";
...
const allData = ref<Thing[]>([])
const filteredData = ref<Thing[]>([])
const keyword = ref('')
...
const modalTitle = computed(() => editingId.value ? '编辑物品' : '新增物品')
...
function goEdit(thing: Thing) {
    editingId.value = thing.id || null
    const versionIds = Array.isArray(thing.version) 
        ? thing.version.map((v: Version) => v.id).filter(Boolean)
        : []
    formThing.value = { 
        ...thing,
        version: versionIds
...
function goDelete(thing: Thing) {
    ElMessageBox.confirm(`删除物品 ${thing.name}`, '删除', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
    }).then(() => {
        if (!thing.id) return
        starveApi.thingDelete({ id: thing.id })
            .then(res => {
                getThingList()
                ElMessage({
...
function thingAddSubmit() {
    const submitData = {
        ...formThing.value,
        version: formThing.value.version.map((id: number) => 
            versionList.value.find(v => v.id === id)
        ).filter(Boolean)
    }
    starveApi.thingAdd(submitData)
        .then(() => {
            ElMessage({
                message: '添加成功',
                type: 'success'
...
function thingModifySubmit() {
    ...
    starveApi.thingModify(submitData)
        .then(() => {
            ElMessage({
                message: '修改成功',
                type: 'success'
